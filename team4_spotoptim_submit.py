...
"""Stock single-LightGBM ``team_4`` SpotOptim baseline, adapted to run inside
this package.

PROVENANCE -- this file started as a copy of a course script
(``bart26l-vorlesung/scripts/team4_spotoptim_submit.py``) that targets a much
newer, unpinned environment: ``spotforecast2-safe >= 22.10.0`` (this package
pins 18.1.0 -- see ``MODEL_CARD.md``) and an external ``challenge_leaderboard``
Python package that is not a dependency here. Importing the pasted-in version
as-is fails immediately (``ModuleNotFoundError: challenge_leaderboard``, plus
several missing sf2-safe symbols: ``SubmissionDates``, ``compute_submission_dates``,
``download_combined_with_fallback``, ``assert_submission_coverage``,
``multitask.guards.assert_no_leakage``, ``processing.shape_check``,
``data.entsoe_loader`` -- none exist in 18.1.0).
...
made ``team_4`` a *different model*: a single stock LightGBM
(``default_lgbm_forecaster_factory``, available unchanged in 18.1.0), fixed
``lags_consider=[1, 2, 24]``, one daily ``Period`` only, no PACF lag selection,
no custom SpotOptim search space (``mt.run(show=False)`` -> the package's
``_default_spotoptim_search_space``), and no weather/holiday/ENTSO-E-provider
exogenous features. This keeps ``team_4`` a useful *local reference baseline*
to diff against the syntaxerror ensemble, without duplicating ~150 lines of
guard logic that must stay in lockstep with syntaxerror_submit.py's own
SYNC NOTE.

NO EXTRA DATA DOWNLOAD NEEDED -- ``team_4``'s ``TRAIN_YEARS = 3`` window fits
entirely inside the already-cached ``data/interim/energy_load.csv``
(2022-01-01 -> present, the same ENTSO-E history syntaxerror_submit.py
downloads daily); this script reuses that same cache/data home
(``PACKAGE_ROOT/data``, ``PACKAGE_ROOT/.cache``) instead of the course
script's separate ``~/spotforecast2_data/ddmo_ch13_live``. ``team_4`` is
already registered in the bundled ``teams.yml``, so no registry edit is
needed either. Weather is fetched live via Open-Meteo on every run (same
mechanism syntaxerror_submit.py already uses) -- nothing to pre-download
there.

WARNING -- ``--push`` opens a real PR against ``bartzbeielstein/challenge-
leaderboard`` (see ``syntaxerror_submit.py``'s ``_push_steps``) under the
identity of ``team_4``, the course's own baseline team, not syntaxerror. Run
this locally for comparison (``--no-validate --no-figures``, optionally
``--skip-download``/``--as-of`` for a historical replay like
``data/backtest/xgb_comparison.md``) -- do not pass ``--push`` unless you are
actually authorized to submit on team_4's behalf.

Usage:

    export ENTSOE_API_KEY=...
    uv run python team4_spotoptim_submit.py --skip-download --no-validate --no-figures
    uv run python team4_spotoptim_submit.py --help
"""
...
# --- team_4 baseline constants (deliberately simple -- see module docstring) --
COUNTRY = se.COUNTRY
DEFAULT_TEAM_ID = "syntaxerror"               # already registered in teams.yml
TRAIN_YEARS = 3
PREDICT_SIZE = 24
REFIT_SIZE = 7
NUMBER_FOLDS = 10
...
def parse_args(argv=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="team4_spotoptim_submit",
        description="Stock single-LightGBM team_4 SpotOptim baseline "
                    "(local reference benchmark for the syntaxerror ensemble).",
    ...
    p.add_argument("--push", action="store_true",
                   help="branch + commit + open a PR as team_4 -- see the "
                        "WARNING in this script's module docstring")
...
def setup_logging(verbosity: int) -> None:
    ...
    handler.setFormatter(logging.Formatter(
        "%(asctime)s [team4_spotoptim_submit] %(levelname)s %(message)s", "%H:%M:%S"))
...
# --- config + pipeline (the only genuinely team_4-specific code) ---------------
def build_config(cov: "se.Coverage", *, n_trials: int, n_initial: int, train_years: int):
    """Stock baseline: single LightGBM, fixed lags, one daily Period, no
    ...
    cfg = ConfigEntsoe(
        ...
        # matching the stock baseline's minimal feature set.
    ...
    # Isolate the model cache from syntaxerror's own ensemble cache
    # ("bart26k-live-team4") so neither run overwrites the other's tuned models.
    cfg.data_frame_name = "bart26k-live-team4-baseline"
...
def _run(args: argparse.Namespace) -> int:
    ...
    logger.info("=" * 72)
    logger.info("team4_spotoptim_submit -- stock single-LightGBM baseline")
    ...
    if not args.no_figures:
        figdir = (args.figures_dir and Path(args.figures_dir).expanduser()) or (
            se.PACKAGE_ROOT / "data" / "figures_team4" / dates.tomorrow.date().isoformat())
